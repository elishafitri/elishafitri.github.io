...
`beginContour()` is a really cool function that essentially cuts out a negative custom shape out of another custom shape.  Therefore it can only be used in `beginShape()`. It was also important for me to note that the vertices for the negative shape would need to go around anti-clockwise for it to work.
